  ...
  async depositToTable(userId: string, amount: number) {
    ...
    if (session.isPlaying) {
      throw new Error('Không thể nạp tiền khi đang trong ván chơi');
    }
  ...
  private async processGuess(
    ...
    const isHighRiskUnder =
      [3, 4, 5].includes(currentCard.card) && choice === 'under';

    const isHighRiskOver =
      [9, 10, 11, 12].includes(currentCard.card) && choice === 'over';
  ...
  async checkJackpotCondition(
    ...
    const isJackpot = last4Cards.every((card) => card.card === 13);
